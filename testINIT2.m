...
tau = 1e-3
itermax = 3000
opts = struct('polym', m, 'tol', 1e-6, 'itmax', itermax, 'chksym', 1, 'nomore', 1, 'vmax', 50);
...
for n_samples = sizes
    ...
    tic;
    for i = 1:repeat
        [eigenvalues, eigenvectors, iters] = CoordinateDescent_triofm_sq(L2, kwant, stepsize, nonzerocols, itermax, V0, w, alpha, tau, batch);
    end
    time = toc/repeat;

    fprintf("--------------------------------------------\n")
    fprintf("results from CoD(sq, init): \n")
    fprintf("running time %.4f \n", time)
    fprintf("#iteration: %10d = %5d * %10d \n", iters*n_sample2, iters, n_sample2)
    fprintf("computed eigenvalues: %f \n", sort(eigenvalues)-a0)
    ...
    tic;
    for i = 1:repeat
        [eigenvalues, eigenvectors, iters] = CoordinateDescent_triofm_sq(L2, kwant, stepsize, nonzerocols, itermax, rand(n_sample2, kwant), w, alpha, tau, batch);
    end
    time = toc/repeat;
