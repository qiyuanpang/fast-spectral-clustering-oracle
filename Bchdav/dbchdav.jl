...
function dbchdav(varargin...)
    ...
    global DA_operator
    DA_operator = distribute(varargin[1], procs = workers(), dist = [length(workers()), 1])
    ...
    if length(varargin) >= (4-Anumeric)
        ...
        if !isa(user_opts,Dict)
            ...
            for key in keys(user_opts)
                if key == "v0"
                    opts[key] = distribute(user_opts[key], procs=workers(), dist=[length(workers()), 1])
                else
    ...
    if  haskey(opts, "v0")
        ...
        if  sizev0 < blk 
            ...
            opts["v0"][:,sizev0+1:blk] = drand((dim,blk-sizev0), workers(), [length(workers()), 1])
        end
    else
        opts["v0"] = drand((dim, blk), workers(), [length(workers()), 1])
        sizev0 = blk
    end
    ...
    V = dzeros((dim, vomax), workers(), [length(workers()), 1])
    W = dzeros((dim, trunc(Int64, vimax)), workers(), [length(workers()), 1]) #note vimax<vomax, inner restart also saves memory
    ...
    while  iter_total <= itmax 
        ...
        Vtmp, orth_flops = DGKS_blk(V, 1:n, dVtmp)
        orth_cputotal = orth_cputotal + CPUtoq()  
        ...
        #
        # if  Anumeric > 0
        sW,_ =  duser_Hx(Matrix{Float64}(V[:, n1:n]))
...
#
    ...
    for k = 1:colv
        ...
        nrmproj = norm(V[:,k],2);  orth_flops=orth_flops+ndim;
    ...
    if  vrank > 0
        ...
        returnV, _ = DGKS_blk(newX, 1:size(newX,2), drand((ndim,colv-vrank), V.pids, [length(V.pids), 1]))
    ...
    if  1==0  #set it to be true only for debugging purpose
        # if (nargin==3),
            if  orthtest==1
                #orthVX = norm(V'*X);
                xorth=norm(hcat(X[:,cols],V)'*hcat(X[:,cols],V) - Matrix{Float64}(size(hcat(X[:,cols],V),2),size(hcat(X[:,cols],V),2)))
                if  xorth > 1e-8
...
function LANCZ_bound(n, varargin...)
    # %
...
#----------------------------------------------------------------------------
function Cheb_filter(x, polm, low, high, augment)
# %   
...
#----------------------------------------------------------------------------
function Cheb_filter_scal(x, polm, low, high, leftb, augment)
# %   
