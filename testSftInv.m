...
for n_samples = sizes
    ...
    tic;
    for i = 1:repeat
        [eigenvalues, eigenvectors, iters] = CoordinateDescent_triofm_sq_si(L, kwant, stepsize, nonzerocols, itermax, V0, w, 0, tau, batch, lambda);
    end
    time = toc/repeat;

    fprintf("--------------------------------------------\n")
    fprintf("results from CoD(sq, shift & invert): \n")
    fprintf("running time %.4f \n", time)
    fprintf("#iteration: %10d = %5d * %10d \n", iters*n_samples, iters, n_samples)
    fprintf("computed eigenvalues: %f \n", sort(eigenvalues)-a0)
    %fprintf("eigV error: %10.4e , loss: %10.4e \n", norm(L*eigenvectors - eigenvectors*diag(eigenvalues), fnorm)/norm(eigenvectors*diag(eigenvalues), fnorm), loss(eigenvectors))
    ...
    fprintf("--------------------------------------------\n")
    fprintf("results from CoD(sq, mmt): \n")
    fprintf("running time %.4f \n", time)
    fprintf("#iteration: %10d = %5d * %10d \n", iters*n_samples, iters, n_samples)
    fprintf("computed eigenvalues: %f \n", sort(eigenvalues)-a0)
    %fprintf("eigV error: %10.4e , loss: %10.4e \n", norm(L*eigenvectors - eigenvectors*diag(eigenvalues), fnorm)/norm(eigenvectors*diag(eigenvalues), fnorm), loss(eigenvectors))
    ...
    tic;
    for i = 1:repeat
        [eigenvalues, eigenvectors, iters] = CoordinateDescent_triofm_sq_si(L, kwant, stepsize, nonzerocols, itermax, V0, w, alpha, tau, batch, lambda);
    end
    time = toc/repeat;

    fprintf("--------------------------------------------\n")
    fprintf("results from CoD(sq, mmt, shift & invert): \n")
    fprintf("running time %.4f \n", time)
    fprintf("#iteration: %10d = %5d * %10d \n", iters*n_samples, iters, n_samples)
    fprintf("computed eigenvalues: %f \n", sort(eigenvalues)-a0)
    %fprintf("eigV error: %10.4e , loss: %10.4e \n", norm(L*eigenvectors - eigenvectors*diag(eigenvalues), fnorm)/norm(eigenvectors*diag(eigenvalues), fnorm), loss(eigenvectors))
